-- FINAL Setup Script - All columns included
...
BEGIN
    ...
    -- Add products (answer = description for products)
    INSERT INTO knowledge_base (business_id, type, product_name, price, description, answer, is_active, created_at, updated_at)
    VALUES
        (business_uuid, 'product', 'Ankara Maxi Dress', 15000, 'Beautiful ankara maxi dresses available in blue, wine, and olive green. Made from quality ankara fabric.', 'Beautiful ankara maxi dresses available in blue, wine, and olive green. Made from quality ankara fabric. Price: ₦15,000 each.', true, NOW(), NOW()),
        (business_uuid, 'product', 'Lace Gown', 25000, 'Elegant lace evening gowns. Available in black, gold, and silver. Perfect for special occasions.', 'Elegant lace evening gowns. Available in black, gold, and silver. Perfect for special occasions. Price: ₦25,000.', true, NOW(), NOW()),
        (business_uuid, 'product', 'Casual T-Shirts', 5000, 'Comfortable cotton t-shirts in various colors and sizes.', 'Comfortable cotton t-shirts in various colors and sizes. Price: ₦5,000 each.', true, NOW(), NOW());
    ...
    RAISE NOTICE '✅ Success! Business created!';
...
SELECT
    type,
    COALESCE(product_name, question, 'N/A') as item,
    COALESCE(price::text, '-') as price,
    LEFT(answer, 60) || '...' as content
FROM knowledge_base
